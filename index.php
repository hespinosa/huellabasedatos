...
if(isset($_POST['submit'])){
    if($_POST['submit'] == 'Conectar'){
        ...
        $result = pg_query($dbconn, "select release,fecha from huella group by release,fecha;");
...
<body>
 ...
 <label>Servidor:</label> <input type="text" id="host" name="host" value="localhost" required>
 <label>Puerto:</label> <input type="text" id="port" name="port" value="5432" required>
 <label>Usuario:</label> <input type="text" id="userss" name="userss" value="postgres" required>
 <label>Clave:</label> <input type="password" id="passss" name="passss" value="root" required>
 <label>Base de Datos:</label> <input type="text" id="database" name="database" value="bank" required>
 ...
 <table border="1" >
		...
		<tr>
			...
			<td><input type="radio" name="radiocomparar" onclick="Selecionar(this);" value="<?php  echo $value['release']; ?>"></td>
		</tr>
        <?php } ?>
        <?php }?>
		</table>
        ...
        <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" >
            ...
            <input type="hidden" id="database" name="database" value="<?php  echo $database; ?>">
            <input type="hidden" id="database" name="database" value="<?php  echo $database; ?>">
            <label>Nombre Nuevo Release:</label> <input type="text" id="newrelease" name="newrelease" required>
 ...
 <table border="1" >
		...
		<tr>
			...
			<td><?php  echo $value02['tipo']; ?></td>
			<td><?php  echo $value02['objecto']; ?></td>
			<td><?php  echo $value02['codigo']; ?></td>
			<td><?php  if($value02['codigo'] == $value02['codigo2']){echo "<span style='background-color: green;'>******* OK *******</span>";} else{echo "<span style='background-color: red;'>** MODIFICADO **</span>";}  ?></td>
		</tr>
        <?php } ?>
        <?php }?>
